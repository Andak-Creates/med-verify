...
export default function CallScreen() {
  ...
  const [unreadCount, setUnreadCount] = useState(1); // Simulated unread messages
  const [messageInput, setMessageInput] = useState('');
  const [messages, setMessages] = useState([
    { id: 1, text: 'Hi doctor, are you there?', sender: 'user', time: '02:00 PM' },
  ]);
        ...
        <Modal visible={chatVisible} animationType="slide" transparent>
          <View style={styles.chatOverlay}>
            ...
            <ScrollView contentContainerStyle={styles.chatList}>
              {messages.map((m) => {
                const isMe = m.sender === 'pharmacist';
                return (
                ...
                <TouchableOpacity
                  ...
                  onPress={() => {
                    if (messageInput.trim()) {
                      setMessages([...messages, { id: Date.now(), text: messageInput, sender: 'pharmacist', time: 'Now' }]);
                      setMessageInput('');
                    }
                  }}
                >
