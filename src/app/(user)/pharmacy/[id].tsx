  ...
  return (
    <SafeAreaView style={styles.container} edges={["top"]}>
      ...
      <View style={styles.header}>
        <Pressable
          onPress={() => router.canGoBack() ? router.back() : router.replace('/(user)/pharmacy' as any)}
      ...
      {isLoading ? (
        ...
        >
          ...
          <Text
            style={{ marginTop: 12, color: "#6B7280", textAlign: "center" }}
          >
            {error ?? "Pharmacist not found."}
          </Text>
          <TouchableOpacity onPress={reload} style={{ marginTop: 12 }}>
            <Text style={{ color: "#312E81", fontWeight: "700" }}>
              Try again
            </Text>
          </TouchableOpacity>
        </View>
        ...
        >
          ...
          <View style={styles.heroSection}>
            <View style={styles.avatarWrap}>
              {pharmacist.profileImage ? (
                <Image
                  source={{ uri: pharmacist.profileImage }}
                  style={styles.avatar}
                />
              ) : (
            ...
            <Text style={styles.docName}>
              {pharmacist.fullName ?? "Pharmacist"}
            </Text>
          ...
          <View style={styles.statsRow}>
            <View style={styles.statItem}>
              ...
              <Text style={styles.statValue}>
                {pharmacist.avgRating > 0
                  ? pharmacist.avgRating.toFixed(1)
                  : "—"}
              </Text>
          ...
          {pharmacist.vacationMode && (
            <View style={[styles.card, { backgroundColor: "#FFF7ED" }]}>
              <Text style={[styles.cardTitle, { color: "#C2410C" }]}>
                Currently Unavailable
              </Text>
              <Text style={styles.cardBody}>
                This pharmacist is on vacation and is not accepting new bookings
                right now.
              </Text>
            </View>
          )}
          ...
          >
            ...
            <Text style={styles.consultPrice}>
              ₦{pharmacist.feeDrugInquiry.toLocaleString()}
            </Text>
            <TouchableOpacity
              style={[
                styles.bookBtn,
                pharmacist.vacationMode && { opacity: 0.5 },
              ]}
              disabled={pharmacist.vacationMode}
              onPress={() => handleBook("audio")}
            >
          ...
          >
            ...
            <View style={styles.consultInfo}>
              <Text style={[styles.consultTitle, styles.consultTitleDark]}>
                Full Consultation
              </Text>
              <Text style={[styles.consultSub, styles.consultSubDark]}>
                Chat + audio call session
              </Text>
            </View>
            ...
            >
              <View style={{ flexDirection: 'row', alignItems: 'center' }}>
                <Text style={styles.bookBtnText}>Book Now</Text>
                <Ionicons name="calendar-outline" size={16} color="#fff" style={{ marginLeft: 6 }} />
              </View>
            </TouchableOpacity>
          </TouchableOpacity>
