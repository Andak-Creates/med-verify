...
export default function DrugDetailsScreen() {
  ...
  return (
    <SafeAreaView style={{ flex: 1, backgroundColor: '#F8FAFC' }} edges={['top']}>
      ...
      <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={{ padding: 20, paddingTop: 8, paddingBottom: 60 }}>
        {/* Title Hero */}
        <View style={{
        ...
        <SectionCard title={t.drugDetails.registryInfo} icon="document-text-outline">
          <InfoRow label={t.result.nafdacNo} value={drug?.nafdacNumber ?? code ?? null} />
          <InfoRow label={t.result.category} value={drug?.category ?? null} />
          <InfoRow label={t.result.form} value={drug?.form ?? null} />
          <InfoRow label={t.result.approvalDate} value={drug?.approvalDate ?? null} />
          <InfoRow label={t.result.registryStatus} value={drug?.registryStatus ?? null} />
          {drug?.manufacturer ? (
            <View style={{ paddingVertical: 8 }}>
              <Text style={{ fontSize: 13, color: '#8E9CB2', fontWeight: '600', marginBottom: 4 }}>{t.result.manufacturer}</Text>
        ...
        {drug?.activeIngredients ? (
          <SectionCard title={t.drugDetails.activeIngredients} icon="flask-outline">
